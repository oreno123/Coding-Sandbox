...
class ContentFetcher:
    ...
    async def fetch_content(self, bvid: str, cid: int = None, title: str = None) -> VideoContent:
        ...
        if not asr_text:
            asr_text, refreshed_cid = await self._try_asr_with_refreshed_cid(bvid, cid)
            if refreshed_cid:
                resolved_cid = refreshed_cid
        if asr_text:
    ...
    async def _try_asr_with_refreshed_cid(self, bvid: str, old_cid: Optional[int]) -> tuple[Optional[str], Optional[int]]:
        """收藏夹接口偶尔不给 cid 或给到资源 id，这里用视频详情刷新 cid 后重试。"""
        try:
            video_info = await self.bili.get_video_info(bvid)
        except Exception as e:
            logger.info(f"[{bvid}] 刷新 cid 失败: {e}")
            return None, None

        fresh_cid = video_info.get("cid")
        if not fresh_cid or fresh_cid == old_cid:
            return None, None

        logger.info(f"[{bvid}] 使用详情接口 cid={fresh_cid} 重试 ASR")
        return await self._try_asr(bvid, fresh_cid), fresh_cid
    ...
    async def fetch_all_videos_content(
        ...
        for i, video in enumerate(videos):
            ...
            ugc = video.get("ugc") or {}
            cid = (
                video.get("cid")
                or ugc.get("first_cid")
                or video.get("first_cid")
                or video.get("page", {}).get("cid")
            )
